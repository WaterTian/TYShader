...
void main() {
   vec2 uv = -(vec2(u_resolution.x-gl_FragCoord.x, gl_FragCoord.y) / u_resolution.xy) * (1. - 2.*zoom) + vec2(zoom, zoom);
   float niceNoise1 = FBM( vec3(80.0 * uv, speed * 60.*u_time));
   float niceNoise2 = FBM( vec3(80.0 * uv, speed * 62.*u_time + 300.));
   gl_FragColor = texture2D(u_video, uv + vec2(magn*0.2*niceNoise1,magn*0.21*niceNoise2) );
}
