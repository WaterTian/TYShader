...
#define randSamp u_texture1
#define colorSamp u_video


vec4 getRand(vec2 pos)
{
    return textureLod(u_texture1,pos/Res1/u_resolution.y*1080., 0.0);
}
...
{
    vec2 pos = fragCoord+4.0*sin(u_time*1.*vec2(1,1.7))*u_resolution.y/400.;
    vec3 col = vec3(0);
    vec3 col2 = vec3(0);
    float sum=0.;
    for(int i=0;i<AngleNum;i++)
    {
        ...
        {
            vec2 dpos  = v.yx*vec2(1,-1)*float(j)*u_resolution.y/400.;
            vec2 dpos2 = v.xy*float(j*j)/float(SampNum)*.5*u_resolution.y/400.;
            vec2 g;
            float fact;
            float fact2;
    ...
    col/=float(SampNum*AngleNum)*.75/sqrt(u_resolution.y);
    col2/=sum;
    col.x*=(.6+.8*getRand(pos*.7).x);
    col.x=1.-col.x;
    col.x*=col.x*col.x;

    vec2 s=sin(pos.xy*.1/sqrt(u_resolution.y/400.));
    vec3 karo=vec3(1);
    karo-=.5*vec3(.25,.1,.1)*dot(exp(-s*s*80.),vec2(1));
    float r=length(pos-u_resolution.xy*.5)/u_resolution.x;
    float vign=1.-r*r*r;
    gl_FragColor = vec4(vec3(col.x*col2*karo*vign),1);
    //gl_FragColor=getCol(fragCoord);
}
